...
inlining pp@(DProgram (DMainDef main stmts) funcs) =
  let p = DProgram (DMainDef main $ inliningMain funcs stmts) []
   in trace ("\n\n" ++ groom pp ++ "\n\n") p
...
sepAgg (DLoopDef e sts) =
  let (defs, newE) = sepAgg' e
      inLoop = concatMap sepAgg sts
      newSt = DLoopDef newE (inLoop ++ defs)
   in defs ++ [newSt]
sepAgg (DIfDef e sts1 sts2) =
  let (defs, newE) = sepAgg' e
...
sepAgg' aggE@(DAggr agg e gen ins dtype) =
  let (defs, newE) = sepAgg' e
...
sepAgg' (DFunAp func es t) =
  let defs = concatMap (fst . sepAgg') es
...
transIf' (DIfDef expr sts1 sts2) = ifSts
  where
    vars = nub $ concatMap getVars sts1 ++ concatMap getVars sts2
    ifSts = map mkSt vars
    ...
    lookup_expr key (DVarDef var _expr : sts)
      | key == var = _expr
