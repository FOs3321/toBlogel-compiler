...
includeStmt = unlines stmts
    where stmts = ["#include \"basic/pregel-dev.h\"",
                   "#include <float.h>",
                   "#include <climits>",
                   "using namespace std;"]
...
mainStmts inputGraphPath outputGraphPath = unlines stmts
    where 
        stmts = [   "void pregel_main(string in_path, string out_path){",
                    "    WorkerParams param;",
...
genWorker steps = unlines m2
    where
        m1 = ["class BlogelWorker:public Worker<BlogelVertex, BlogelAgg>{"
            ,"    char buf[100];"
            ,"    public:"
        ...
        m2 = ["class BlogelWorker:public Worker<BlogelVertex, BlogelAgg>{"
            ,"    char buf[100];"
            ,"    public:"
            ,"        virtual BlogelVertex* toVertex(char* line){"
            ,"            char * pch;"
...
genCombiner msg = unlines $ head ++ m ++ tail
    where
        head = ["class BlogelCombiner:public Combiner<Message>{",
                "    public:",
...
genBroadcast msg step = head ++ tail
    where
        head = ("if(" ++ codeMsgVs step ++ "){")
                : (indent (["vector<VertexID> & nbs=value().edges;",
                                　"for(int i=0; i<nbs.size(); i++){"]
                                　++ indent (["Message msg;"] ++ m)))
        tail = ["        "++ gg msg ++"send_message(nbs[i], msg);",
                "    }",
                "}"]
...
genGetAggAndMsg agg msg = getAgg ++ getMsg
    where
        getAgg =["void getAggregator(){"] 
                ++ indent (["Agg* agg=(Agg*)getAgg();"]
                ++ map f agg)
                ++ ["}"]
                where f (mem,_,_) = "value()." ++ mem ++ "=agg->" ++ mem ++ ";"
        getMsg = ["void getMessage(MessageContainer & messages){"]
                 ++ indent ( (map g msg)
                             ++ ["for (int i = 0; i < messages.size(); i++){"]
                             ++ indent (map f msg)
                             ++ ["}"]   )    
                 ++ ["}"]
                where f (mem,_,DAggr ag _ _ _ _) = codeDAgg ag ("value()." ++ mem) ("messages[i]." ++ mem )
...
genCompute steps msg = head ++ stps ++ tail
    where 
        head = ["virtual void compute(MessageContainer & messages){",
                "    if(step_num()==1){",
                "        value().nextStep = 0;",
                -- "        wakeAll();",
                "    }else{",
...
cExpr (DFunAp f args t) gen= DFunAp f (map (\e->cExpr e gen) args) t
cExpr e _= e
