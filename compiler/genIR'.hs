...
mkStep' ss =
  ( IRStep (snum ss) (reverse $ sstmts ss) (sconditons ss) (ssendMsg ss),
...
genIR' :: String -> [DStatement] -> IR
genIR' pname stmts = IR pname vertex message aggregator steps
  where
    vertex' = genVertexStruct stmts
    vertex = trace ("\nvars\n" ++ groom vertex' ++ "\n") vertex'
    (steps, message, aggregator) = genSteps' stmts

genSteps' :: [DStatement] -> ([IRStep], IRMsg, IRAgg)
genSteps' stmts = trace ("\n\n\nUsed Vars\n" ++ show (susedVars ss) ++ "\n\n") (sortOn stpNum stps, msg, agg)
  where
...
genVertexStruct :: [DStatement] -> IRStruct
genVertexStruct stmts = nub $ map (f . extractVar) $ concatMap filterVarDef stmts
  where
    extractVar (DVarDef v _) = v
    filterVarDef (DVarDef v e) = [DVarDef v e]
    filterVarDef (DLoopDef _ sts) = concatMap filterVarDef sts
    filterVarDef (DIfDef _ sts1 sts2) = concatMap filterVarDef sts1 ++ concatMap filterVarDef sts2
    filterVarDef _ = []
