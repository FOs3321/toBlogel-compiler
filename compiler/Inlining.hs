...
inliningMain  funcs ast = addElseSts$inliningMain' ast []
    where inliningMain' _ast oldast | _ast==oldast = _ast
                                     | otherwise = inliningMain'  newast _ast
                    where newast =concatMap (analizeLSt funcs) _ast
addElseSts:: [DStatement]->[DStatement]
addElseSts  =map addElseSt
    where addElseSt (DVarDef var expr)  = DVarDef var (addElseExpr expr var)
                where addElseExpr (DIf expr1 expr2 IfNone None)  _var = DIf expr1 expr2 (DVExp _var None) None
...
analizeLExpr funcs (DIf expr1 expr2 IfNone  vtype)  = (DIf expr1' expr2' IfNone vtype,sts)
    where ([expr1',expr2'], sts) =foldl (\(x,y) (x1, y1) ->(x++[x1], y++y1)) ([],[]) $ map (analizeLExpr  funcs) [expr1,expr2]

analizeLExpr funcs (DIf expr1 expr2 expr3  vtype)  = (DIf expr1' expr2' expr3' vtype,sts)
    where ([expr1',expr2',expr3'], sts) =foldl (\(x,y) (x1, y1) ->(x++[x1], y++y1)) ([],[]) $ map (analizeLExpr  funcs) [expr1,expr2,expr3]
analizeLExpr funcs (DCmprhnsnExp expr1  varset dt)  = (DCmprhnsnExp expr1' varset dt, sts)
    where (expr1', sts) =analizeLExpr  funcs expr1
...
lookupvarC  key (DFunDef (DFunV fun var ft)  expr   : xys) | key ==fun = Just (expr, var)
