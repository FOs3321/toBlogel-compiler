...
data StepState = StepState { stmts :: [DStatement],
                             steps :: [IRStep],
                             msg :: IRMsg,
                             agg :: IRAgg,
                             stepNum :: Int,
...
genSteps' stmts = let (_, stps, msg, agg, _, _) = foldl genStep' ([],[],[],[], 0, []) stmts
        ...
        DAggr dagg aggExpr gen ins dtype ->
            case gen of
                DGenG vtx graph gtype ->
                    if needSepStp sts vardef then let step = IRStep i (reverse sts) (IRStpID (i + 1)) isSend
                                                      newStps = step : stps
                                                      newAgg = (getVarName vname, dtype, expr)
                                                      newAggs = if newAgg `elem` agg then agg else newAgg:agg
                                                  in ([vardef],newStps, msg, newAggs, i + 1, [])
                                             else let newAgg = (getVarName vname, dtype, expr)
                                                      newAggs = if newAgg `elem` agg then agg else newAgg:agg
                ...
                DGenNvals vtx graph gtype ->
                    ...
                    in if needSepStp sts vardef then let step = IRStep i (reverse sts) (IRStpID $ i + 1) (isSend ++ sendMs ins)
                                                         newStps = step : stps
                                                         newMsg = (getVarName vname, dtype', expr)
                                                         newMsgs = if newMsg `elem` msg then msg else newMsg:msg
                                                     in ([vardef],newStps, newMsgs, agg, i + 1, [])
                                                else let newMsg = (getVarName vname, dtype', expr)
                                                         newMsgs = if newMsg `elem` msg then msg else newMsg:msg
...
isSendTrue vname (step:t) = (IRStep i sts c (vs ++ vname)):t
    where (IRStep i sts c vs) = step

checkIsSend :: [IRStep] -> [DVar]
checkIsSend steps = vs
    where (IRStep _ _ _ vs) = head $ reverse steps
...
setSts ((IRStep i sts cond b):steps) st = (step:steps)
    where step = IRStep i (sts ++ [st]) cond b

genVertexStruct :: [DVar] -> IRStruct
genVertexStruct env = map f env
    where f (DVar name _ dtype) = (name, dtype)
